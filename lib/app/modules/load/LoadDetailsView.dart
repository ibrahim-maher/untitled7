  ...
  void _checkPermissions() {
    ...
    canDeleteLoad.value = isOwner && (
        currentLoad.status == LoadStatus.posted ||
            ...
            currentLoad.status == LoadStatus.draft
    );
...
👤 Contact: ${currentLoad.contactPerson ?? 'N/A'}
📞 Phone: ${currentLoad.contactPhone ?? 'N/A'}
...
''';
  ...
  void callContactPerson() async {
    final currentLoad = load.value;
    if (currentLoad == null || currentLoad.contactPhone?.isEmpty == true) {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 4),
                Text('ETA: ${_daysDifference(DateTime.now(), bid.estimatedDelivery as DateTime)} days'),
              ],
            ),
          ],
        ),
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  radius: 24,
                  backgroundColor: Theme.of(Get.context!).primaryColor.withOpacity(0.1),
                  backgroundImage: bid.transporterPhotoUrl != null
                      ? NetworkImage(bid.transporterPhotoUrl!)
                      : null,
                  child: bid.transporterPhotoUrl == null
                      ? Icon(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(Icons.star, color: Colors.amber[600], size: 16),
                          const SizedBox(width: 4),
                          Text('${bid.transporterRating} rating'),
                          const SizedBox(width: 12),
                          Text('${bid.completedTrips} trips'),
                        ],
                      ),
                    ],
                  ),
            ...
            const SizedBox(height: 20),
            _buildBidDetailItem('Bid Amount', '₹${bid.bidAmount.toStringAsFixed(0)}'),
            _buildBidDetailItem('Vehicle Type', bid.vehicleType),
            _buildBidDetailItem('Vehicle Number', bid.vehicleNumber),
            _buildBidDetailItem('Estimated Pickup', _formatDate(bid.estimatedPickup)),
            _buildBidDetailItem('Estimated Delivery', _formatDate(bid.estimatedDelivery as DateTime)),
            _buildBidDetailItem('Submitted', _formatDateTime(bid.createdAt)),
  ...
  void _contactTransporter(BidModel bid) async {
    // Since transporterPhone is not in your model, we'll need to get it from user data
    // For now, show a placeholder message
    _showInfoSnackbar('Contact feature will be implemented with phone numbers');
  }
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Do you want to accept the bid from ${bid.transporterName} for ₹${bid.bidAmount.toStringAsFixed(0)}?',
            ),
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(12),
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    _buildAnalyticsItem(
                      'Views',
                      '${analytics.value?.views ?? load.value?.viewCount ?? 0}',
                      Icons.visibility,
                    ),
                    _buildAnalyticsItem(
                      'Bids Received',
  ...
  String _calculateAverageBid() {
    ...
    final total = bids.fold<double>(0, (sum, bid) => sum + bid.bidAmount);
    return (total / bids.length).toStringAsFixed(0);
  }

  String _calculateResponseRate() {
    final views = analytics.value?.views ?? load.value?.viewCount ?? 0;
    if (views == 0) return '0';
...
// LoadAnalytics model for analytics data (matches your service)
class LoadAnalytics {
  final int views;
  final int shares;
  final int inquiries;
